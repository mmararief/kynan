...
function getFilteredData($tanggal = null)
{
  global $koneksi;
  if ($tanggal) {
    $sql = "SELECT * FROM pengeluaran WHERE tanggal = :tanggal ORDER BY tanggal DESC";
...
function exportToExcel()
{
  ...
  $sheet->setCellValue('D1', 'Sumber Pengeluaran');
  ...
  $filename = 'Daftar_Pengeluaran.xlsx';
...
function exportToPDF()
{
  ...
  $pdf->SetTitle('Daftar Pengeluaran');
  $pdf->SetSubject('Pengeluaran');
  $pdf->SetKeywords('TCPDF, PDF, pengeluaran');
  ...
  $pdf->Output('Daftar_Pengeluaran.pdf', 'D');
  exit;
}
...
if ($_GET['aksi'] == 'tambah') {
  ...
  if (isset($_POST['tanggal']) && isset($_POST['keterangan']) && isset($_POST['sumber']) && isset($_POST['jumlah'])) {
    ...
    // Buat query SQL untuk menambahkan data ke dalam tabel pengeluaran
    $sql = 'INSERT INTO pengeluaran (tanggal, keterangan, sumber, jumlah) VALUES (?, ?, ?, ?)';
    ...
    if ($row->execute($data)) {
      ...
      echo '<script>
                    Swal.fire({
                        icon: "success",
                        title: "Sukses Tambah Pengeluaran",
                    }).then(() => {
                        window.location = "pengeluaran.php?success=tambah";
                    });
                </script>';
      ...
      echo '<script>
                    Swal.fire({
                        icon: "error",
                        title: "Gagal Tambah Pengeluaran",
...
if (isset($_GET['aksi']) && $_GET['aksi'] == 'edit') {
  // Jika permintaan adalah untuk mengedit pengeluaran
  $id = $_GET['id'];
  $tanggal = $_POST['tanggal']; // Ambil data nama pengeluaran yang baru dari form
  $keterangan = $_POST['keterangan'];
  $sumber = $_POST['sumber'];
  $jumlah = $_POST['jumlah'];

  $sql = "UPDATE pengeluaran SET tanggal = ?, keterangan = ?, sumber = ?, jumlah = ?
          WHERE id_pengeluaran = ?";
  $row = $koneksi->prepare($sql);
  $row->execute(array($tanggal, $keterangan, $sumber, $jumlah, $id));

  // Tampilkan alert setelah mengedit pengeluaran
  echo '<script src="https://cdn.jsdelivr.net/npm/sweetalert2@11"></script>';
  echo '<script>
        Swal.fire({
            icon: "success",
            title: "Sukses Edit Pengeluaran",
        }).then(() => {
...
if (!empty($_GET['aksi'] == 'hapus')) {
  ...
  echo '<script>
      Swal.fire({
          icon: "success",
          title: "Sukses Hapus Pengeluaran",
      }).then(() => {
          window.location = "pengeluaran.php";
      });
  </script>';
